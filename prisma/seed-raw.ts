/**
 * Seed script menggunakan raw MariaDB queries langsung.
 * Digunakan karena PrismaMariaDb adapter memiliki issue dengan pool timeout
 * saat running lokal (koneksi remote ke IndoGlobal).
 */
...
async function main() {
  ...
  try {
    // ── USERS ──────────────────────────────────────────────────────────────
    const adminPass = await bcrypt.hash('Admin@BPS2024!', 10);
    ...
    const users = [
      ...
      { email: 'pegawai@bps.go.id', password: adminPass, name: 'Pegawai BPS', role: 'BPS_ADMIN', instansi: 'BPS Provinsi/Kabupaten' },
    ...
    for (const u of users) {
      await conn.query(
        `INSERT INTO users (email, password, name, role, instansi)
         VALUES (?, ?, ?, ?, ?)
         ON DUPLICATE KEY UPDATE password=VALUES(password), name=VALUES(name), instansi=VALUES(instansi)`,
        [u.email, u.password, u.name, u.role, u.instansi]
      );
    ...
    for (const m of masterData) {
      await conn.query(
        ...
        [m.nik, m.nama_lengkap, m.alamat_lengkap]
      );
      seeded++;
    }
    ...
    console.log('  👨‍💻 Pegawai BPS      : pegawai@bps.go.id         | Admin@BPS2024!');
